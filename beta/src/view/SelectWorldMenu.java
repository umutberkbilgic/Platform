	...
	public void update(GameContainer gc, StateBasedGame sbg, int arg) throws SlickException {
		...
		if(input.isMousePressed(0)) {
			if(playButton.isPositionContained(xpos, ypos))
				...
				sbg.enterState(1);
			else if(earthButton.isPositionContained(xpos, ypos))
			{
				((view.GameState)sbg.getState(9)).setWorldNo(1);
				sbg.enterState(5);
			}
			else if(moonButton.isPositionContained(xpos, ypos))
			{
				((view.GameState)sbg.getState(9)).setWorldNo(2);
				sbg.enterState(5);
			}
			else if(marsButton.isPositionContained(xpos, ypos))
			{
				((view.GameState)sbg.getState(9)).setWorldNo(3);
				sbg.enterState(5);
			}
		}
	}
